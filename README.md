# B超生成新生儿照 - 微信小程序

基于PRD和API文档构建的完整微信小程序项目。

## 📁 项目结构

```
detectBaby/
├── miniprogram/                 # 小程序代码
│   ├── app.js                   # 入口文件
│   ├── app.json                 # 全局配置
│   ├── app.wxss                 # 全局样式
│   ├── pages/                   # 页面目录
│   │   ├── home/                # 首页
│   │   ├── upload/              # 上传页
│   │   ├── validate/            # 校验页
│   │   ├── unlock/              # 解锁页
│   │   ├── job/                 # 生成中页
│   │   ├── result/              # 结果页
│   │   ├── history/             # 历史记录页
│   │   └── settings/            # 设置页
│   └── images/                  # 图片资源
├── cloudfunctions/              # 云函数目录
│   ├── login/                   # 登录获取openid
│   ├── validateUltrasound/      # B超校验
│   ├── unlockPrepare/           # 准备解锁会话
│   ├── unlockClaimAd/           # 广告解锁
│   ├── unlockClaimPay/          # 支付解锁
│   ├── orderCreate/             # 创建订单
│   ├── wxPrepay/                # 微信预支付
│   ├── wxPayNotify/             # 支付回调
│   ├── generationCreate/        # 创建生成任务
│   ├── generationGet/           # 查询任务状态
│   ├── assetGet/                # 获取资产链接
│   ├── originalDownload/        # 获取原图
│   ├── galleryHome/             # 首页参考图
│   ├── userHistory/             # 用户历史
│   └── userDataDelete/          # 删除用户数据
├── project.config.json          # 项目配置
└── README.md                    # 说明文档
```

## 🚀 快速开始

### 1. 配置云开发环境

1. 打开微信开发者工具
2. 导入本项目
3. 在 `project.config.json` 中填写你的 AppID
4. 开通云开发，获取环境ID
5. 在 `miniprogram/app.js` 中替换环境ID

### 2. 创建数据库集合

在云开发控制台创建以下集合：

- `uploads` - 上传记录
- `ultrasound_validations` - 校验记录
- `unlock_sessions` - 解锁会话
- `generation_jobs` - 生成任务
- `assets` - 资产
- `orders` - 订单
- `payments` - 支付记录
- `gallery_items` - 参考图库

### 3. 部署云函数

在微信开发者工具中，右键每个云函数目录，选择"上传并部署"。

### 4. 配置微信支付（可选）

如需使用微信支付功能：
1. 在微信支付商户平台完成配置
2. 在云开发控制台关联微信支付
3. 配置支付回调云函数

### 5. 配置激励视频广告（可选）

1. 在微信公众平台申请广告位
2. 获取广告单元ID
3. 在 `pages/unlock/unlock.js` 中替换广告单元ID

## 📱 功能说明

### 核心流程

1. **上传B超图** → 选择/拍照图片
2. **图片校验** → 检测是否为B超图、是否含敏感信息
3. **解锁生成** → 观看广告或付费¥9.9
4. **AI生成** → 生成写实宝宝照片
5. **保存/下载** → 保存展示图或解锁原图

### 计费规则

- **生成**：广告免费 或 ¥9.9/次（含原图）
- **原图下载**：广告解锁 或 ¥9.9/次

### 数据保留

- 默认保留30天
- 用户可一键删除所有数据

## 🔧 配置项

### 环境变量

在 `app.js` 中修改：

```javascript
// 云开发环境ID
env: 'your-env-id'

// 价格配置（分）
price: {
  gen_with_original_1: 990,  // ¥9.9
  unlock_original_1: 990      // ¥9.9
}
```

### 广告配置

在 `pages/unlock/unlock.js` 中修改：

```javascript
// 激励视频广告单元ID
adUnitId: 'your-ad-unit-id'
```

## 📋 数据库索引建议

```
uploads: openid+created_at, sha256
ultrasound_validations: openid+created_at, uploadFileID
unlock_sessions: openid+created_at, expires_at, unlock_token
generation_jobs: openid+created_at, status, job_id
assets: job_id
orders: openid+created_at, order_id
payments: order_id, wx_txn_id(唯一)
gallery_items: enabled+sort
```

## ⚠️ 注意事项

1. **AI生成服务**：当前代码使用模拟生成，实际项目需接入AI Image Provider
2. **广告验证**：生产环境应验证广告观看是否完整
3. **支付安全**：确保支付回调验签正确配置
4. **隐私合规**：确保隐私政策、用户协议符合法规要求

## 📄 许可证

本项目仅供学习参考，商业使用请遵守相关法律法规。
